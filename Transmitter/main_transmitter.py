...
def generate_caption(image_path):
    """Call the caption generator script and return the generated caption."""
    try:
        result = subprocess.run(['python', 'caption_generator.py', image_path], capture_output=True, text=True)
        caption = result.stdout.strip()
        if result.returncode == 0:
            return caption
        else:
            print(f"Error generating caption: {result.stderr}")
            return None
    except Exception as e:
        print(f"Exception occurred while generating caption: {e}")
        return None
...
def main():
    ...
    node = setup_loRa()
    desktop_path = "/home/raspberry/Desktop/IoT_Project"
    if not os.path.exists(desktop_path):
        print(f"Directory {desktop_path} does not exist. Exiting.")
        sys.exit(1)
            ...
            try:
                ...
                if "capture" in words.lower():
                    with picamera.PICamera() as camera:
                        ...
                        if capture_image(camera, image_path):
                            caption = generate_caption(image_path)
                            if caption:
                                send_message(node, caption)
                                print(f"Caption sent: {caption}")
