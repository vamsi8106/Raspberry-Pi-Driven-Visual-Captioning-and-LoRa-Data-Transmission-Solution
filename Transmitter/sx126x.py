...
# "20,868,Hello World"
    ...
    def audio(self):
        ...
        audio_file_path = os.path.join(desktop_path, "received_message.mp3")
        ...
        try:
            ...
            pygame.event.wait()  # Wait for the playback to finish
    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            ...
            received_message = str(r_buff[3:-1])
            # Save the received message to a text file on the desktop
            text_file_path = os.path.join(desktop_path, "received_messages.txt")
            with open(text_file_path, "a") as file:
                file.write(received_message + '\n')
                        # Convert the received message to an audio file
            
            audio_file_path = os.path.join(desktop_path, "received_message.mp3")
            tts = gTTS(text=received_message, lang='en')
            tts.save(audio_file_path)
            print(f"Message saved to {text_file_path} and {audio_file_path}")
